...
async function run() {
  ...
  const payload = seed.map((n, i) => ({ nama: n, urutan: i + 3 })); // since 1 and 2 already used for Bandung and Batang
  const { data, error } = await supabase.from("sumber_donasi").insert(payload);
  console.log("Insert result:", error ? error.message : "Success seeded data.");
}
run();
